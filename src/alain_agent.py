"""Phase 6 AlainAgent implementation with OpenAI Agents SDK."""
...
ALAIN_SYSTEM_PROMPT = "Algebraic structures specialist. You identify key concepts and explain them with enthusiasm"
...
def check_paper_tool(
    ...
    "Use this tool to check if Algebra papers in the requested date range have already been collected."
    if len(categories) == 0:
        categories = list(ALAIN_CATEGORIES)
...
def arxiv_fetcher_tool(
    ...
    """
    Fetch papers in AlainAgent's allowed categories.

    Use this tool when there is no Algebra paper in the requested date range.
    ...
    categories_set = set(categories).intersection(ALAIN_CATEGORIES)
    if not categories_set:
        categories_set = set(ALAIN_CATEGORIES)
    ...
    categories_ordered = [category for category in ALAIN_CATEGORIES if category in categories_set]
...
def build_alain_agent() -> Agent:
    """Create AlainAgent for phase 6."""
    return Agent(
        name="AlainAgent",
        instructions=(
            f"{ALAIN_SYSTEM_PROMPT}\n"
            f"Allowed categories only: {', '.join(ALAIN_CATEGORIES)}.\n"
            "You communicate with passion.\n"
            ...
            "When you answer back to JuliusAgent, you **must** return a JSON with this exact schema:"
            f"{{'AlainAgent': [{TOPIC_REPRESENTATION_DICT}]}}\n"
...
def run_alain_agent(message: str) -> Dict[str, Any]:
    """Run one AlainAgent turn with SDK tracing enabled."""
    ...
    agent = build_alain_agent()
    with trace(
        "phase6-alain-agent-run",
        metadata={
            "agent": "AlainAgent",
...
def _classify_categories_with_llm(message: str) -> List[str]:
    """Use an LLM to classify the requested AlainAgent categories."""
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    model = os.getenv("OPENAI_MODEL", "gpt-4.1-mini")
    prompt = (
        "Classify the user's message into arXiv categories from this allowed set only: "
        f"{list(ALAIN_CATEGORIES)}.\n"
        "Return JSON only with this schema: {\"categories\": <list of predicted categories>}\n"
        "Information:\n"
        "- 'math.AG' contains algebraic geometry papers.\n"
        "- 'math.AT' contains algebraic topology papers.\n"
        "- 'math.RA' contains rings and algebras papers.\n"
        "- 'math.GR' contains group theory papers.\n"
        f"User message: {message}"
    )
    response = client.responses.create(
        model=model,
...
def _get_arxiv_categories(message: str) -> List[str]:
    """
    Infer AlainAgent categories from the user message.

    Returns only categories allowed to AlainAgent and defaults to all allowed
    categories when no explicit or implicit signal is found.
    """
    try:
    ...
    normalized = [category for category in ALAIN_CATEGORIES if category in raw_categories]
    if not normalized:
        return list(ALAIN_CATEGORIES)
    return normalized
